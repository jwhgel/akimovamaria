...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // 5 _  Отрисовка графика y1 = x (Замените на ваш график)
        drawGraph(window, [](float x) { return x; }, -100, 100, 100, 100, sf::Color::Blue);

        // 5 _   Отрисовка графика y2 = -x (Замените на ваш график)
        drawGraph(window, [](float x) { return -x; }, -100, 100, 100, 100, sf::Color::Red);
